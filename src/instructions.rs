...
impl Instruction for AdcAN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: ADC A, {:#04X}", cpu.get_pc(), n);
        cpu.inc_pc(2);
    }
}
...
impl Instruction for AddAN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: ADD A, {:#04X}", cpu.get_pc(), n);
        cpu.inc_pc(2);
    }
}
...
impl Instruction for AddAR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: ADD A, {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for AddHlSs {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: ADD HL, {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for AddAMemIyD {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        if d & 0b10000000 != 0 {
            d = (d ^ 0xFF) + 1;
            trace!("{:#06x}: ADD A, (IY-{:#04X})", curr_pc - 1, d);
        } else {
            trace!("{:#06x}: ADD A, (IY+{:#04X})", curr_pc - 1, d);
        }
        cpu.inc_pc(2);
...
impl Instruction for AndR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: AND {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for AndN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: AND {:#04X}", cpu.get_pc(), n);
        cpu.inc_pc(2);
    }
}
...
impl Instruction for BitBMemIyD {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        if d & 0b10000000 != 0 {
            d = (d ^ 0xFF) + 1;
            trace!("{:#06x}: BIT {}, (IY-{:#04X})", curr_pc - 2, self.b, d);
        } else {
            trace!("{:#06x}: BIT {}, (IY+{:#04X})", curr_pc - 2, self.b, d);
        }
        cpu.inc_pc(2);
...
impl Instruction for CallNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: CALL {:#06X}", cpu.get_pc(), nn);
        cpu.set_pc(nn);
    }
}
...
impl Instruction for CallCcNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: CALL {:?}, {:#06X}", curr_pc, self.cond, nn);
        if condval {
            curr_pc += 3;
            cpu.write_word(curr_sp - 1, ((curr_pc & 0xFF00) >> 8) as u8);
            cpu.write_word(curr_sp - 2,  (curr_pc & 0x00FF)       as u8);
...
impl Instruction for CpR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: CP {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for CpMemIyD {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        if d & 0b10000000 != 0 {
            d = (d ^ 0xFF) + 1;
            trace!("{:#06x}: CP (IY-{:#04X})", curr_pc - 1, d);
        } else {
            trace!("{:#06x}: CP (IY+{:#04X})", curr_pc - 1, d);
        }
        cpu.inc_pc(2);
...
impl Instruction for DecSs {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: DEC {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for DecIyD {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        if d & 0b10000000 != 0 {
            d = (d ^ 0xFF) + 1;
            trace!("{:#06x}: DEC (IY-{:#04X})", curr_pc - 1, d);
        } else {
            trace!("{:#06x}: DEC (IY+{:#04X})", curr_pc - 1, d);
        }
        cpu.inc_pc(2);
...
impl Instruction for Djnz {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: DJNZ {:#06X}", cpu.get_pc(), target);
        if bval > 0 {
            cpu.set_pc(target);
        } else {
            cpu.inc_pc(2);
...
impl Instruction for Im {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: IM {}", cpu.get_pc() - 1, self.mode);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for InAPortN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: IN A, ({:#04X})", cpu.get_pc(), n);
        cpu.inc_pc(2);
    }
}
...
impl Instruction for IncR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: INC {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for JpNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: JP {:#06X}", cpu.get_pc(), nn);
        cpu.set_pc(nn);
    }
}
...
impl Instruction for JpCcNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: JP {:?}, {:#06X}", cpu.get_pc(), self.cond, nn);
        if condval {
            cpu.set_pc(nn);
        } else {
            cpu.inc_pc(3);
...
impl Instruction for JrZ {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: JR Z, {:#06X}", cpu.get_pc(), target);
...
impl Instruction for JrNz {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: JR NZ, {:#06X}", cpu.get_pc(), target);
        if cpu.get_flag(ZERO_FLAG) {
            cpu.inc_pc(2);
        } else {
            cpu.set_pc(target);
...
impl Instruction for JrNcE {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: JR NC, {:#06X}", cpu.get_pc(), target);
...
impl Instruction for JrCE {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: JR C, {:#06X}", cpu.get_pc(), target);
...
impl Instruction for JrE {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: JR {:#06X}", cpu.get_pc(), target);
        cpu.set_pc(target);
    }
}
...
impl Instruction for LdRN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD {:?}, {:#04X}", cpu.get_pc(), self.r, n);
        cpu.inc_pc(2);
    }
}
...
impl Instruction for LdDdNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD {:?}, {:#06X}", cpu.get_pc(), self.r, nn);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for LdHlMemNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD HL, ({:#06X})", cpu.get_pc(), nn);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for LdMemHlN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD (HL), {:#04X}", cpu.get_pc(), n);
        cpu.inc_pc(2);
    }
}
...
impl Instruction for LdRMemIyD {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        if d & 0b10000000 != 0 {
            d = (d ^ 0xFF) + 1;
            trace!("{:#06x}: LD {:?}, (IY-{:#04X})", curr_pc - 1, self.r, d);
        } else {
            trace!("{:#06x}: LD {:?}, (IY+{:#04X})", curr_pc - 1, self.r, d);
        }
...
impl Instruction for LdMemIyDN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        if d & 0b10000000 != 0 {
            d = (d ^ 0xFF) + 1;
            trace!("{:#06x}: LD (IY-{:#04X}), {:#04X}", curr_pc - 1, d, n);
        } else {
            trace!("{:#06x}: LD (IY+{:#04X}), {:#04X}", curr_pc - 1, d, n);
        }
...
impl Instruction for LdIxNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD IX, {:#06X}", cpu.get_pc() - 1, nn);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for LdIxMemNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD IX, TODO {:#06X}", cpu.get_pc() - 1, nnmemval);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for LdMemNnIx {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD ({:#06X}), IX", cpu.get_pc() - 1, nn);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for LdMemIxDN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        if d & 0b10000000 != 0 {
            d = (d ^ 0xFF) + 1;
            trace!("{:#06x}: LD (IX-{:#04X}), {:#04X}", curr_pc - 1, d, n);
        } else {
            trace!("{:#06x}: LD (IX+{:#04X}), {:#04X}", curr_pc - 1, d, n);
        }
...
impl Instruction for LdRR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD {:?}, {:?}", cpu.get_pc(), self.rt, self.rs);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for LdMemNnDd {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD ({:#06X}), {:?}", cpu.get_pc() - 1, nn, self.r);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for LdMemHlR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD (HL), {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for LdMemNnA {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD ({:#06X}), A", cpu.get_pc(), nn);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for LdMemNnHl {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD ({:#06X}), HL", cpu.get_pc(), nn);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for LdAMemNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD A, ({:#06X})", cpu.get_pc(), nn);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for LdIyNn {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: LD IY, {:#06X}", cpu.get_pc() - 1, nn);
        cpu.inc_pc(3);
    }
}
...
impl Instruction for OrR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: OR {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for OutPortCR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: OUT (C), {:?}", cpu.get_pc() - 1, self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for OutPortNA {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: OUT ({:#04X}), A", cpu.get_pc(), n);
        cpu.inc_pc(2);
    }
}
...
impl Instruction for PopQq {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: POP {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for PushQq {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: PUSH {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for ResBMemIyD {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        if d & 0b10000000 != 0 {
            d = (d ^ 0xFF) + 1;
            trace!("{:#06x}: RES {}, (IY-{:#04X})", curr_pc - 2, self.b, d);
        } else {
            trace!("{:#06x}: RES {}, (IY+{:#04X})", curr_pc - 2, self.b, d);
        }
        cpu.inc_pc(2);
...
impl Instruction for ResBMemHl {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: RES {}, (HL)", cpu.get_pc() - 1, self.b);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for RetCc {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: RET {:?}", cpu.get_pc(), self.cond);
...
impl Instruction for RlR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: RL {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for Rst {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: RST {:#04X}", cpu.get_pc(), self.addr);
...
impl Instruction for SetBMemIyD {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        if d & 0b10000000 != 0 {
            d = (d ^ 0xFF) + 1;
            trace!("{:#06x}: SET {}, (IY-{:#04X})", curr_pc - 2, self.b, d);
        } else {
            trace!("{:#06x}: SET {}, (IY+{:#04X})", curr_pc - 2, self.b, d);
        }
        cpu.inc_pc(2);
...
impl Instruction for SetBMemHl {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: SET {}, (HL)", cpu.get_pc() - 1, self.b);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for SbcR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: SBC A, {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for SubN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: SUB {:#04X}", cpu.get_pc(), n);
        cpu.inc_pc(2);
    }
}
...
impl Instruction for SubR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: SUB {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for XorR {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: XOR {:?}", cpu.get_pc(), self.r);
        cpu.inc_pc(1);
    }
}
...
impl Instruction for XorN {
    fn execute(&self, cpu: &mut Cpu) {
        ...
        trace!("{:#06x}: XOR {:#04X}", cpu.get_pc(), n);
        cpu.inc_pc(2);
    }
}
